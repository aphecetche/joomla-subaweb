...
<div class="contentopen">
	<form method="post">
        ...
        <?php if ($this->item->require_email || $this->item->show_email):?>
            <div class="osdownloadsemail"><?php echo(JText::_("Email"));?> (*): <input type="text" name="require_email" id="require_email" /></div>
        <?php endif;?>
        ...
        <div class="osdownloadsactions">
            ...
            <div class="btn download">
                <a href="<?php echo(JRoute::_("index.php?option=com_osdownloads&task=getdownloadlink&tmpl=component&id={$this->item->id}"));?>"  id="btn_download" style="color:<?php echo($this->item->download_color);?>"  class="readmore"><span><?php echo($this->item->download_text ? $this->item->download_text : JText::_("Download"));?></span></a>
            </div>
        </div>
...
$javascript .= "\n" . 'if (Validate()) { var request=this.href; ';

$javascript .= "\n" . 'if  ($("require_email")) { request += "&email=" + $("require_email").value; };';
			
$javascript .= "\n" . 'SqueezeBox.open(request, {onClose: function onClose() {directPage();},';

$javascript .= "\n" . 'handler: "iframe", size: {x:' . $params->get("width", 650) .', y:' . $params->get("height", 350) .'}';
...
if ( $this->item->direct_page ) $javascript .=	'	window.location = ' . $this->item->direct_page . ';';
...
$javascript .= "\n" . 'msg += "' . JText::_("You have agree terms to download this") . '";';

if ($this->item->require_email)
{
$javascript .= "\n" .	'	$("require_email").value = $("require_email").value.trim();';

$javascript .= "\n" . 'if ($("require_email") && ($("require_email").value == "" || !reg.test($("require_email").value)))';

$javascript .= "\n" . 'msg +=' . '"' . JText::_("You have input correct email to get download link") . '";';
}
else {
$javascript .= "\n" .	'$("require_email").value = $("require_email").value.trim();';
    
$javascript .= "\n" . 'if ($("require_email") && ($("require_email").value != "" && !reg.test($("require_email").value)))';
$javascript .= "\n" . 'msg += ' . JText::_("You must input correct email to get download link");
}
